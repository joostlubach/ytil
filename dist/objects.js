...
//------
// objectEquals
export function objectEquals(left, right, equals = isEqual) {
    if (left == null) {
        return right == null;
    }
    if (right == null) {
...
export function modifyObject(root, path, modifier) {
    const segments = path.split('.').filter(Boolean);
    return modify(root, segments, modifier);
}
function modify(obj, segments, modifier) {
...
//------
// modifyInObject
export function modifyInObject(root, path, modify) {
...
export function deepMapValues(arg, fn) {
    if (isPlainObject(arg)) {
        const result = {};
        for (const [attribute, value] of Object.entries(arg)) {
